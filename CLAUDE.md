...
| `principal_360_cycles/{cycleId}` · `principal_360_responses/{respId}` · `principal_360_aggregates/{cycleId}` | Anonymous 360 cycle. Responses persist NO respondent uid (framework anonymity guarantee — NOT an Induction Charter NN; "NN5" label retired 2026-08-08). Aggregates threshold-gated (5+ per cohort). Cloud Function planned. | various — see root CLAUDE.md |
...
- Communications: `AcademicCalendar` (`/academic-calendar`), `announcements`, `messageboard` (`/message-board`), `documents`. All 4 adopted canonical `.page-footer` + 3-part contract (data-page-accent body tint + family-accent CTA gradient) in 2026-05-24 design-system pass. Family = Communication / cyan.
- CPD: `CompetencyFramework` (`/competency-framework`), `LearningPath` (`/learning-path`), `MyPortfolio` (`/my-portfolio`), `MyCertificates` (`/my-certificates`). CompetencyFramework + MyPortfolio adopted canonical `.page-footer` + info-strip explainer (2026-05-23/25). MyPortfolio token fixes shipped at the same time (competency-evidence upload pipeline). LearningPath received the v3 enriched block modal (sticky sidebar + levels-track + 9-block modal + in-modal admin reading override editor) 2026-05-23 — same shape as CH and TH for cross-hub parity.
- Induction: `MyInduction` (`/my-induction`), `TeamInduction` (`/team-induction`), `ObservationEntry` (`/observation-entry`), `handbook`. `handbook` adopted the canonical CH-shared bookshelf strip + spine modal + canonical `.page-footer` + info strip (2026-05-25/26). Page title renamed "Induction Handbook" → "Handbooks" 2026-05-26.
- **References (2026-05-09, parity refactor 2026-05-23/25):** `references` — narrowed AH variant (3 tabs · 16 docs). Now consumes the shared `shared-design/references-shell` (CSS + ES module) for UI parity with CH and TH. Modal viewer auto-wires CTS / SKL / PIGP / ES / AICF chips via `cambridge-crossref.js`. **Local references-data mirror landed 2026-05-25** — previously fetched cross-origin from `centralhub.eduversal.org/references-data/*`; now `build.js` copies references-data into `dist/` so AH no longer has a cross-origin SPOF. Hero + body + footer all hit a true 1200 px content rail (see root CLAUDE.md Common Mistakes #53 + #54 for the alignment / `body:has()` flex trap).
- **AI Maturity Self-Assessment (2026-05-26):** `ai-maturity-self-assessment` — lifecycle info-strip + canonical `.page-footer` adopted. Part of the AICF Phase-2 surfaces (root CLAUDE.md "AI Competency Framework" section).
...
- **Never put `<body style="display:none">` on AH pages.** AH `auth-guard.js` toggles `document.body.style.visibility` (`hidden` → `visible`), not `display`. `display:none` is stronger than `visibility:visible` and the auth-guard's flip cannot override it — page stays permanently blank. Default to plain `<body>` (no inline style); auth-guard manages the pre-auth flash via `visibility`. **Mass-generated pages risk multiplication of this bug** — 2026-05-21 incident: 7 School Leadership placeholder pages (`school-leadership-*` + `school-artifacts` + `school-activities`) all shipped from one shared template that had `<body style="display:none">`, all permanently blank in production until removed. Review the template's body tag before mass-generating. The only legitimate use is the `display:none` + paired `document.body.style.display = ''` override pattern used by `references.html` — but skip it unless you're inheriting from an existing page that already does both halves.
