...
| `ah_categories/{catId}` | Dashboard category collection. Each: `{name, color, cardIds[], visible_to[], hidden_for_users, order, createdAt}`. **Meta doc** `_uncategorized_settings_` (`isMeta:true`, `order:-1`, `visible_to_users:bool`) gates the Uncategorized accordion for non-admins. Non-admin path subscribes via direct `onSnapshot(doc(..., '_uncategorized_settings_'))` (the meta doc lacks `visible_to`, so the open-to-all + sub-role queries miss it). | academic_admin / central_admin |
...
- **Category collection:** `ah_categories`. Each: `{ name, color, cardIds[], visible_to[], hidden_for_users, order, createdAt }`
