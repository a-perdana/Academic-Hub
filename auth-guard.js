...
// competency, induction, aicf, students_assessment, curriculum}. Missing field = all enabled (back-compat);
...
const PILOT_SLUG_MAP = {
  ...
  // ai-maturity-admin (CH) is NOT here — it's an HQ triage tool, admin-gated.
  'ai-validate-teacher-assessments': 'aicf',
  'ai-maturity-self-assessment':     'aicf',
  // Induction track
  'my-induction':       'induction',
  'team-induction':     'induction',
  'observation-entry':  'induction',
  // Students Hub Assessment track (own pilot system since 2026-06-27). AH's
  // school-leadership view of the chapter-test pipeline — student roster
  // approvals + the school-wide chapter-mastery roll-up. Hidden when the
  ...
  // References" column alongside /references and must stay reachable regardless
  // of a school's induction-pilot enrolment. Tagging it 'induction' hid every
  // handbook for any school with induction opted out (all schools currently
  // have enabled_systems:[] = all off). Removed 2026-05-29.
};
...
// approval_status_academichub: 'pending'. Sends an email to a fixed
// recipient (currently secondary.edu@eduversal.org — change in
// PENDING_NOTIFICATION_RECIPIENT below). Body comes from
// mail_templates/ah_pending_notification — admin-editable in CH
// /mail-composer "System Templates". Built-in default used if the
// admin has not yet customised the template doc. Mirrors TH
// notifyAdminsOfPendingTHUser — keep both in sync.
const PENDING_NOTIFICATION_RECIPIENT = 'secondary.edu@eduversal.org';

const PENDING_NOTIFICATION_DEFAULT_SUBJECT =
  'New Academic Hub signup awaiting approval — {{userName}}';

const PENDING_NOTIFICATION_DEFAULT_BODY = `
<p>Hi team,</p>
<p>A new user has signed up to the <strong>Academic Hub</strong> and is waiting for approval.</p>
...
// CH/TH auth-guard helper — keep all three in sync.
...
async function notifyAdminsOfPendingAHUser(user, database) {
  ...
  try {
    const snap = await getDoc(doc(database, 'mail_templates', 'ah_pending_notification'));
    if (snap.exists()) {
      const t = snap.data();
  ...
  try {
    await callMailRelay(user, {
      ...
      payload: {
        ...
        tags: [
          { name: 'kind',     value: 'ah-pending-notification' },
          { name: 'platform', value: 'academichub' },
        ],
      },
    });
